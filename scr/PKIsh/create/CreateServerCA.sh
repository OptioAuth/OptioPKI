...
#
# Company: OptioAuth
# Name: CreateServerCA.sh
# Purpose: Create the server key (server.key.pem) signing uses extensions server_cert 
...
openssl ca -config root/intermediate/intermediate_openssl.cnf -extensions server_cert -days 90 -notext -md sha256 -in server/csr/server.csr.pem -out server/certs/server.cert.pem
